...
const CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card-thumb { height: 160px; background: var(--green-light); display: flex; align-items: center; justify-content: center; font-size: 56px; position: relative; overflow: hidden; }
  ...
  .planner-grid { display: grid; grid-template-columns: 72px repeat(7, 1fr); gap: 6px; }
  .planner-day-header { border-radius: 14px; padding: 10px 6px; text-align: center; background: var(--green); color: white; }
  .planner-day-header.today { background: var(--terra); }
  .planner-day-header .day-name { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; opacity: 0.8; }
  .planner-day-header .day-num { font-size: 20px; font-weight: 700; font-family: 'Playfair Display', serif; line-height: 1.1; }
  .planner-day-header .day-month { font-size: 9px; opacity: 0.65; margin-top: 1px; }
  .planner-day-header .day-cal { font-size: 9px; opacity: 0.65; margin-top: 2px; }
  .planner-meal-label { display: flex; align-items: center; justify-content: flex-end; padding-right: 8px; font-size: 10px; font-weight: 700; color: var(--muted); text-transform: uppercase; letter-spacing: 0.5px; }
  .planner-cell { background: white; border-radius: 12px; min-height: 76px; border: 2px dashed var(--border); display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s; padding: 6px; text-align: center; gap: 3px; position: relative; }
  ...
  .planner-cell .cell-img { width: 32px; height: 32px; border-radius: 8px; object-fit: cover; }
  .planner-cell .cell-emoji { font-size: 20px; }
  .planner-cell .cell-name { font-size: 9px; font-weight: 600; color: var(--text); line-height: 1.2; }
  .planner-cell .cell-cal { font-size: 8px; color: var(--muted); }
  .planner-cell .cell-remove { position: absolute; top: 3px; right: 3px; width: 15px; height: 15px; background: var(--terra); border-radius: 50%; color: white; font-size: 8px; display: none; align-items: center; justify-content: center; border: none; }
  .planner-cell.filled:hover .cell-remove { display: flex; }
  .planner-cell .cell-add { font-size: 18px; color: var(--border); }
  .week-nav { display: flex; align-items: center; gap: 10px; margin-bottom: 14px; }
  .week-nav-btn { width: 34px; height: 34px; border-radius: 10px; border: 1.5px solid var(--border); background: white; font-size: 16px; color: var(--green); transition: all 0.15s; display: flex; align-items: center; justify-content: center; }
  ...
  .week-today-btn { padding: 6px 14px; border-radius: 100px; border: 1.5px solid var(--border); background: white; font-size: 12px; font-weight: 600; color: var(--muted); transition: all 0.15s; margin-left: auto; }
  ...
  .img-tab { flex: 1; padding: 8px; border-radius: 9px; border: none; font-size: 12px; font-weight: 600; color: var(--muted); background: transparent; transition: all 0.15s; }
  ...
  .photo-upload-area:hover { border-color: var(--sage); background: var(--green-light); }
  .photo-upload-area.has-photo { border-style: solid; border-color: var(--sage); padding: 8px; }
  .photo-preview { width: 100%; height: 140px; object-fit: cover; border-radius: 10px; display: block; }
  ...
  .photo-change-btn { margin-top: 8px; font-size: 12px; color: var(--muted); background: none; border: none; text-decoration: underline; cursor: pointer; }
  ...
  .tiktok-input { padding: 12px 16px; border-radius: 12px; border: 2px solid var(--border); font-size: 14px; background: var(--bg); transition: border-color 0.15s; width: 100%; }
  ...
  .modal { background: white; border-radius: 24px; width: 100%; max-width: 520px; max-height: 88vh; overflow-y: auto; box-shadow: var(--shadow-lg); animation: slideUp 0.25s ease; }
  .modal.wide { max-width: 640px; }
  @keyframes slideUp { from { transform: translateY(20px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
  .modal-header { display: flex; align-items: center; gap: 16px; padding: 20px 24px; border-bottom: 1px solid var(--border); position: sticky; top: 0; background: white; border-radius: 24px 24px 0 0; z-index: 1; }
  .modal-thumb { width: 64px; height: 64px; background: var(--green-light); border-radius: 16px; display: flex; align-items: center; justify-content: center; font-size: 36px; overflow: hidden; flex-shrink: 0; }
  ...
  .section-title { font-size: 12px; font-weight: 700; color: var(--muted); text-transform: uppercase; letter-spacing: 0.8px; margin-bottom: 10px; margin-top: 20px; }
  ...
  .modal-edit-btn { padding: 12px 20px; border-radius: 12px; background: var(--terra-light); color: var(--terra); border: 1.5px solid #e8c4b0; font-size: 14px; font-weight: 600; white-space: nowrap; }
  .modal-edit-btn:hover { background: var(--terra); color: white; border-color: var(--terra); }
  .pick-list { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; max-height: 56vh; overflow-y: auto; padding-right: 4px; }
  ...
  .pick-thumb { font-size: 22px; width: 40px; height: 40px; background: var(--bg); border-radius: 10px; display: flex; align-items: center; justify-content: center; overflow: hidden; flex-shrink: 0; }
  ...
  .weekly-summary { background: var(--green); color: white; border-radius: 16px; padding: 16px 20px; margin-bottom: 16px; display: flex; gap: 24px; align-items: center; }
        ...
        <div>
          ...
          {value?.type==='photo'&&value?.photoUrl?(
            ...
            <div className="photo-upload-area" onClick={()=>fileRef.current.click()}>
              ...
              <div className="photo-upload-text">Click to upload a photo</div>
              <div className="photo-upload-sub">JPG, PNG, WEBP supported</div>
            </div>
          )}
        </div>
      )}
...
function RecipeThumb({ recipe, size=56 }) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal wide" onClick={e=>e.stopPropagation()}>
        <div className="modal-header">
          ...
          <div><h2 style={{fontSize:18}}>Edit Recipe</h2><div className="modal-meta">Changes update everywhere</div></div>
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        <div className="modal-header">
          ...
          <div><h2>{recipe.name}</h2><div className="modal-meta">⏱ {recipe.prepTime} min · 🔧 {recipe.cookMethod} · 🌍 {recipe.origin}</div></div>
          <button className="modal-close" onClick={onClose}>✕</button>
        </div>
        <div className="modal-body">
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{maxWidth:480}} onClick={e=>e.stopPropagation()}>
        <div className="modal-header" style={{padding:"16px 20px"}}>
          <div><h2 style={{fontSize:18}}>Pick a Recipe</h2><div className="modal-meta">{label} · {meal}</div></div>
          <button className="modal-close" onClick={onClose}>✕</button>
        </div>
          ...
          <div className="pick-list">
            {filtered.map(r=>(
              <div key={r.id} className="pick-item" onClick={()=>{onPick(r);onClose();}}>
                <div className="pick-thumb"><RecipeThumb recipe={r} size={22}/></div>
...
function DiscoverView({ recipes, onView, onAddToPlanner, onEdit }) {
  ...
  return (
    <div>
      ...
      {shown.length===0?(<div className="empty-state"><div className="empty-icon">🔍</div><h3>No recipes match</h3><p>Try adjusting your filters</p></div>):(
...
function PlannerView({ recipes, weeklyPlan, setWeeklyPlan, onShowShopping }) {
  ...
  function goToday(){ const d=new Date(); d.setHours(0,0,0,0); setWeekStart(d); }

  const totalCals = useMemo(()=>weekDays.reduce((sum,d)=>{
    const day=weeklyPlan[dateKey(d)]||{};
    return sum+Object.values(day).reduce((s,r)=>s+(r?.calories||0),0);
  },0),[weeklyPlan,weekDays]);
  const totalMeals = useMemo(()=>weekDays.reduce((sum,d)=>{
    const day=weeklyPlan[dateKey(d)]||{};
  ...
  return (
    <div>
      ...
      <div className="weekly-summary">
        ...
        <div><div className="s-val">{totalCals}</div><div className="s-lbl">Week Calories</div></div>
        <div className="divider"/>
      ...
      <div style={{overflowX:"auto"}}>
        <div className="planner-grid" style={{minWidth:680}}>
          <div/>
          {weekDays.map(d=>{
            const key=dateKey(d);
            ...
            return (
              <div key={key} className={`planner-day-header ${isToday(d)?"today":""}`}>
                ...
                <div className="day-cal">{dayCal>0?`${dayCal} cal`:""}</div>
              </div>
            );
          })}
          {MEALS.map(meal=>(
...
function ShoppingView({ weeklyPlan }) {
  ...
  if(allIngredients.length===0) return (
    <div><div className="page-header"><h1>Shopping List 🛒</h1></div>
    <div className="empty-state"><div className="empty-icon">🛒</div><h3>No meals planned yet</h3><p>Add recipes to your weekly plan first</p></div></div>
  );
  return (
    <div>
...
function ImportView({ onSave, toast }) {
  ...
  async function handleImport(){
    if(!url&&!desc){setError("Please enter a URL or describe the recipe.");return;}
    setLoading(true);setError("");setResult(null);
    try {
      const prompt=`Generate a healthy calorie-deficit recipe as JSON based on: ${url||desc}\n\nReturn ONLY valid JSON with fields: name, emoji, calories, protein, carbs, fat, prepTime, cookMethod, origin, type, ingredients (array of {n,q}), steps (array), tags (array). No extra text.`;
  ...
  return (
    <div>
      <div className="page-header"><h1>Import from TikTok 🎵</h1><p>Describe a recipe and AI will generate it</p></div>
      <div className="import-box">
        <div style={{marginBottom:16,padding:12,background:"var(--terra-light)",borderRadius:12,fontSize:13,color:"var(--terra)",fontWeight:500}}>⚠️ TikTok URLs can't be fetched directly — describe the recipe you saw below.</div>
...
function MyRecipesView({ recipes, onView, onAddToPlanner, onEdit }) {
  ...
  if(userRecipes.length===0) return (
    ...
    <div className="empty-state"><div className="empty-icon">📖</div><h3>No custom recipes yet</h3><p>Create a recipe or import from TikTok</p></div></div>
  );
  return (
    <div>
...
export default function App() {
  ...
  function updateRecipe(updated){
    ...
    setWeeklyPlan(plan=>{
      ...
      Object.keys(newPlan).forEach(key=>{
        MEALS.forEach(meal=>{if(newPlan[key]?.[meal]?.id===updated.id) newPlan[key]={...newPlan[key],[meal]:updated};});
      });
      return newPlan;
    });
  ...
  const NAV=[
    {id:"discover",icon:"🥗",label:"Discover"},{id:"planner",icon:"📅",label:"Planner"},
    {id:"shopping",icon:"🛒",label:"Shopping"},{id:"myrecipes",icon:"📖",label:"My Recipes"},
    {id:"create",icon:"✏️",label:"Create"},{id:"import",icon:"🎵",label:"Import"},
  ];
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        {selectedRecipe&&<RecipeModal recipe={selectedRecipe} onClose={()=>setSelectedRecipe(null)} onAddToPlanner={r=>{setAddingRecipe(r);setView("planner");setSelectedRecipe(null);}} onEdit={r=>{setEditingRecipe(r);setSelectedRecipe(null);}}/>}
