...
const CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nav-btn {
    ...
    gap: 3px; color: rgba(255,255,255,0.5); transition: all 0.2s; font-size: 18px;
    position: relative;
  }
  ...
  .nav-btn.active::before {
    ...
    width: 4px; height: 24px; background: var(--terra); border-radius: 0 4px 4px 0;
  }
  ...
  .recipe-card {
    ...
    box-shadow: var(--shadow); transition: all 0.25s; cursor: pointer; border: 1px solid var(--border);
  }
  .recipe-card:hover { transform: translateY(-4px); box-shadow: var(--shadow-lg); }
  .card-thumb {
    height: 160px; background: var(--green-light); display: flex; align-items: center;
    justify-content: center; font-size: 56px; position: relative;
  }
  .card-thumb .origin-badge {
    position: absolute; top: 10px; right: 10px; background: rgba(255,255,255,0.92);
    border-radius: 20px; padding: 3px 10px; font-size: 11px; font-weight: 600; color: var(--muted);
  }
  .card-thumb .type-badge {
    position: absolute; top: 10px; left: 10px; background: var(--terra);
    border-radius: 20px; padding: 3px 10px; font-size: 11px; font-weight: 600; color: white;
  }
  ...
  .stat-pill {
    flex: 1; background: var(--bg); border-radius: 10px; padding: 8px 4px; text-align: center;
  }
  ...
  .btn-sm {
    flex: 1; padding: 8px; border-radius: 10px; border: none; font-size: 12px; font-weight: 600;
    transition: all 0.2s;
  }
  ...
  .filter-chip:hover:not(.active) { border-color: var(--sage); color: var(--green); }
  .filter-select {
    padding: 6px 14px; border-radius: 100px; border: 1.5px solid var(--border);
    background: white; font-size: 12px; font-weight: 500; color: var(--text); cursor: pointer;
    appearance: none; padding-right: 28px; background-image: url("data:image/svg+xml,%3Csvg width='10' height='6' viewBox='0 0 10 6' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1L5 5L9 1' stroke='%238A8078' stroke-width='1.5'/%3E%3C/svg%3E");
    background-repeat: no-repeat; background-position: right 10px center;
  }
  .filter-select:focus { outline: none; border-color: var(--green); }
  .randomize-btn {
    margin-left: auto; padding: 8px 20px; border-radius: 100px; background: var(--terra);
    color: white; border: none; font-size: 13px; font-weight: 600; display: flex; align-items: center;
  ...
  .planner-day-header {
    background: var(--green); color: white; border-radius: 12px; padding: 10px 6px;
    text-align: center;
  }
  ...
  .planner-cell {
    background: white; border-radius: 12px; min-height: 80px; border: 2px dashed var(--border);
    display: flex; flex-direction: column; align-items: center; justify-content: center;
    cursor: pointer; transition: all 0.2s; padding: 8px; text-align: center; gap: 4px;
    position: relative;
  }
  ...
  .planner-cell .cell-remove {
    position: absolute; top: 4px; right: 4px; width: 16px; height: 16px;
    background: var(--terra); border-radius: 50%; color: white; font-size: 9px;
  ...
  .shopping-item {
    ...
    background: white; border-radius: 12px; margin-bottom: 6px; border: 1px solid var(--border);
  }
  ...
  .form-input {
    ...
    font-size: 14px; background: white; transition: border-color 0.15s; color: var(--text);
  }
  ...
  .btn-add-ingredient {
    padding: 8px 16px; border-radius: 8px; background: var(--green-light); color: var(--green);
    border: 1.5px dashed var(--sage); font-size: 13px; font-weight: 600; width: 100%; margin-top: 4px;
  ...
  .btn-submit {
    padding: 12px 32px; border-radius: 12px; background: var(--green); color: white;
    border: none; font-size: 15px; font-weight: 600; transition: all 0.2s; margin-top: 8px;
  }
  .btn-submit:hover { background: var(--green-mid); }

  /* IMPORT */
  .import-box {
    background: white; border-radius: 20px; padding: 32px; max-width: 580px; margin: 0 auto;
    border: 1px solid var(--border); box-shadow: var(--shadow);
  }
  .tiktok-input-row { display: flex; gap: 10px; }
  .tiktok-input {
    flex: 1; padding: 12px 16px; border-radius: 12px; border: 2px solid var(--border);
    font-size: 14px; background: var(--bg); transition: border-color 0.15s;
  }
  .tiktok-input:focus { outline: none; border-color: var(--terra); }
  .btn-import {
    padding: 12px 24px; border-radius: 12px; background: var(--terra); color: white;
    border: none; font-size: 14px; font-weight: 600; transition: all 0.2s; white-space: nowrap;
  }
  .btn-import:hover { background: #B04E20; }
  .import-result {
    margin-top: 24px; padding: 20px; background: var(--green-light);
    border-radius: 16px; border: 1.5px solid var(--sage);
  }
  .loading-dots::after {
  ...
  .modal-header {
    ...
    border-bottom: 1px solid var(--border); position: sticky; top: 0; background: white; border-radius: 24px 24px 0 0;
  }
  ...
  .modal-close { margin-left: auto; width: 36px; height: 36px; border-radius: 10px; border: 1.5px solid var(--border); background: transparent; font-size: 18px; color: var(--muted); }
  ...
  .modal-add-btn {
    width: 100%; padding: 12px; border-radius: 12px; background: var(--green); color: white;
    border: none; font-size: 14px; font-weight: 600; margin-top: 20px; display: flex;
  ...
  .toast {
    ...
    background: var(--green); color: white; padding: 12px 24px; border-radius: 100px;
  ...
  .weekly-summary {
    background: var(--green); color: white; border-radius: 16px; padding: 16px 20px;
...
function RecipeCard({ recipe, onView, onAddToPlanner }) {
  return (
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <button className="modal-add-btn" onClick={() => { onAddToPlanner(recipe); onClose(); }}>
            📅 Add to Meal Plan
          </button>
        </div>
      </div>
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{maxWidth:480}} onClick={e=>e.stopPropagation()}>
        <div className="modal-header" style={{padding:"16px 20px"}}>
          <div>
            <h2 style={{fontSize:18}}>Pick a Recipe</h2>
            <div className="modal-meta">{day} · {meal}</div>
          </div>
          <button className="modal-close" onClick={onClose}>✕</button>
        </div>
            ...
            {filtered.map(r => (
              <div key={r.id} className="pick-item" onClick={() => { onPick(r); onClose(); }}>
                ...
                <div>
                  ...
                  <div className="pick-cal">{r.calories} kcal · {r.protein}g protein</div>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p>Low-calorie, high-protein meals for your deficit goals</p>
      </div>
      ...
      <div style={{display:"flex",gap:10,marginBottom:16}}>
        ...
        <button className="randomize-btn" onClick={() => setDisplayed(randomize())}>🎲 Randomize</button>
      ...
      <div className="filter-row">
        ...
        <select className="filter-select" value={filters.origin} onChange={e=>setFilters(f=>({...f,origin:e.target.value}))}>
      ...
      {shown.length === 0 ? (
        ...
        <div className="recipe-grid">
          {shown.map(r => <RecipeCard key={r.id} recipe={r} onView={onView} onAddToPlanner={onAddToPlanner}/>)}
        </div>
      )}
    </div>
  );
...
function PlannerView({ recipes, weeklyPlan, setWeeklyPlan, onShowShopping }) {
  ...
  function assignRecipe(recipe) {
    setWeeklyPlan(p => ({ ...p, [pendingDay]: { ...p[pendingDay], [pendingMeal]: recipe } }));
  }
  function removeRecipe(day, meal) {
    setWeeklyPlan(p => ({ ...p, [day]: { ...p[day], [meal]: null } }));
  }

  const totalCals = Object.values(weeklyPlan).reduce((sum, day) =>
    sum + Object.values(day).reduce((s, r) => s + (r?.calories || 0), 0), 0);
  const totalMeals = Object.values(weeklyPlan).reduce((sum, day) =>
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p>Plan your deficit meals for the week ahead</p>
      </div>
      <div className="weekly-summary">
        ...
        <div style={{marginLeft:"auto"}}>
          <button className="btn-import" style={{background:"rgba(255,255,255,0.2)",border:"1.5px solid rgba(255,255,255,0.4)"}} onClick={onShowShopping}>
            🛒 Shopping List
          </button>
        </div>
      </div>
                  ...
                  <div key={day} className={`planner-cell ${recipe?"filled":""}`}
                    ...
                    {recipe ? (
                      <>
                        ...
                        <button className="cell-remove" onClick={e=>{e.stopPropagation();removeRecipe(day,meal);}}>✕</button>
                      </>
      ...
      {picking && (
        <PickRecipeModal recipes={recipes} day={pendingDay} meal={pendingMeal}
          onPick={r => assignRecipe(r)} onClose={() => setPicking(false)}/>
      )}
    </div>
  );
}
...
function ShoppingView({ weeklyPlan }) {
    ...
    Object.values(weeklyPlan).forEach(day =>
      Object.values(day).forEach(r => {
        ...
        r.ingredients.forEach(ing => {
          ...
          if (!map[key]) map[key] = { name: ing.n, qty: ing.q, recipes: [r.name] };
          else if (!map[key].recipes.includes(r.name)) map[key].recipes.push(r.name);
        });
      })
    );
  ...
  if (allIngredients.length === 0) return (
    <div>
      ...
      <div className="empty-state">
        ...
        <p>Add recipes to your weekly plan to generate a shopping list</p>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="page-header">
        <h1>Shopping List 🛒</h1>
        <p>{allIngredients.length} ingredients from your meal plan</p>
      </div>
      <div className="shopping-actions">
        <button className="btn-secondary" onClick={() => setChecked({})}>Clear All</button>
        <button className="btn-secondary" onClick={copyList}>📋 Copy List</button>
      ...
      <div style={{background:"white",borderRadius:20,padding:28,border:"1px solid var(--border)",maxWidth:720}}>
        <div style={{marginBottom:20}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {EMOJIS.map(e=>(
              <button key={e} onClick={()=>setForm(f=>({...f,emoji:e}))} style={{width:40,height:40,fontSize:22,border:`2px solid ${form.emoji===e?"var(--green)":"var(--border)"}`,borderRadius:10,background:form.emoji===e?"var(--green-light)":"white"}}>{e}</button>
            ))}
          </div>
        </div>
        ...
        <div className="form-grid">
          <div className="form-group full">
            ...
            <input className="form-input" placeholder="e.g. Spicy Tuna Bowl" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))}/>
          </div>
          <div className="form-group">
            <label className="form-label">Calories (kcal) *</label>
            <input className="form-input" type="number" placeholder="350" value={form.calories} onChange={e=>setForm(f=>({...f,calories:e.target.value}))}/>
          </div>
          <div className="form-group">
            <label className="form-label">Protein (g) *</label>
            <input className="form-input" type="number" placeholder="30" value={form.protein} onChange={e=>setForm(f=>({...f,protein:e.target.value}))}/>
          </div>
          <div className="form-group">
            <label className="form-label">Carbs (g)</label>
            <input className="form-input" type="number" placeholder="20" value={form.carbs} onChange={e=>setForm(f=>({...f,carbs:e.target.value}))}/>
          </div>
          <div className="form-group">
            <label className="form-label">Fat (g)</label>
            <input className="form-input" type="number" placeholder="10" value={form.fat} onChange={e=>setForm(f=>({...f,fat:e.target.value}))}/>
          </div>
          <div className="form-group">
            <label className="form-label">Prep Time (min)</label>
            <input className="form-input" type="number" placeholder="20" value={form.prepTime} onChange={e=>setForm(f=>({...f,prepTime:e.target.value}))}/>
          </div>
          <div className="form-group">
        ...
        <div className="form-group" style={{marginTop:16}}>
          ...
          {form.ingredients.map((ing,i) => (
            <div key={i} className="ingredient-row">
              ...
              <input className="form-input" style={{flex:1}} placeholder="Qty (e.g. 200g)" value={ing.q} onChange={e=>updateIng(i,"q",e.target.value)}/>
        ...
        <div className="form-group" style={{marginTop:16}}>
          ...
          <textarea className="form-input form-textarea" placeholder={"Preheat oven to 200°C.\nSeason the chicken.\nBake for 25 mins."} value={form.steps} onChange={e=>setForm(f=>({...f,steps:e.target.value}))}/>
        </div>
...
Return ONLY a valid JSON object with these exact fields:
{
  ...
  "cookMethod": "one of: Baked|Grilled|Stir-fried|Pan-fried|Boiled|Raw/Poached|Frozen",
  "origin": "cuisine origin",
  "type": "Savory or Dessert",
  "ingredients": [{"n": "ingredient name", "q": "quantity"}],
  "steps": ["step 1", "step 2", ...],
  "tags": ["tag1", "tag2"]
}
...
Make it a realistic healthy, low-calorie high-protein recipe. Return ONLY the JSON, no other text.`;
      ...
      setError("Could not generate recipe. Try describing it differently.");
    }
    setLoading(false);
  }
  ...
  return (
    <div>
      <div className="page-header"><h1>Import from TikTok 🎵</h1><p>Paste a TikTok video URL and AI will generate the recipe</p></div>
      <div className="import-box">
        <div style={{marginBottom:16,padding:12,background:"var(--terra-light)",borderRadius:12,fontSize:13,color:"var(--terra)",fontWeight:500}}>
          ⚠️ Note: Due to browser restrictions, TikTok URLs can't be fetched directly. The AI will generate a recipe inspired by the URL context or your description below.
        </div>
        ...
        <div className="form-group" style={{marginBottom:16}}>
          <label className="form-label">Or describe the recipe you saw</label>
          <textarea className="form-input form-textarea" style={{fontSize:13}} placeholder="e.g. A TikTok recipe for high-protein cottage cheese pasta with cherry tomatoes and basil, super simple 4 ingredient meal..." value={desc} onChange={e=>setDesc(e.target.value)}/>
        </div>
        ...
        {result && (
          <div className="import-result">
            ...
            <div style={{fontSize:12,color:"var(--muted)",marginBottom:12}}>
              <strong>{result.ingredients?.length} ingredients:</strong> {result.ingredients?.map(i=>i.n).join(", ")}
            </div>
            <button className="btn-submit" style={{width:"100%"}} onClick={saveRecipe}>Save to My Recipes ➕</button>
          </div>
        )}
      </div>
    </div>
...
function MyRecipesView({ recipes, onView, onAddToPlanner, onDelete }) {
  ...
  if (userRecipes.length === 0) return (
    <div>
      ...
      <div className="empty-state">
        ...
        <p>Create a recipe or import from TikTok to see it here</p>
      </div>
    </div>
  );
  return (
    <div>
      ...
      <div className="recipe-grid">
        {userRecipes.map(r => <RecipeCard key={r.id} recipe={r} onView={onView} onAddToPlanner={onAddToPlanner}/>)}
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const [addingRecipe, setAddingRecipe] = useState(null); // recipe to add to planner

  function showToast(msg) { setToast(msg); }
  function saveRecipe(recipe) {
    const tagged = { ...recipe, tags: [...(recipe.tags||[]), "Custom"] };
    setRecipes(r => [...r, tagged]);
  }
  function importRecipe(recipe) {
    const tagged = { ...recipe, tags: [...(recipe.tags||[]), "Imported"] };
    setRecipes(r => [...r, tagged]);
  }
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        <main className="main">
          {view==="discover" && <DiscoverView recipes={recipes} onView={setSelectedRecipe} onAddToPlanner={r=>{setAddingRecipe(r);setView("planner");}} />}
          ...
          {view==="myrecipes" && <MyRecipesView recipes={recipes} onView={setSelectedRecipe} onAddToPlanner={r=>{setAddingRecipe(r);setView("planner");}} />}
          {view==="create" && <CreateRecipeView onSave={saveRecipe} toast={showToast}/>}
          {view==="import" && <ImportView onSave={importRecipe} toast={showToast}/>}
        </main>

        {selectedRecipe && <RecipeModal recipe={selectedRecipe} onClose={()=>setSelectedRecipe(null)} onAddToPlanner={r=>{setAddingRecipe(r);setView("planner");setSelectedRecipe(null);}}/>}
